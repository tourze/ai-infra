...
- **Git 锁文件处理**：若 `git` 因 `.git/index.lock` 失败，必须先用 `lsof .git/index.lock` 与 `ps` 确认没有活跃 Git 进程占用；仅在确认锁为陈旧锁后，才可删除并重试。禁止在未检查占用的情况下直接删除锁文件。
- **复杂 `rg` 正则先单独验证**：包含分组、转义、字符类或多重 alternation 的 `rg` 正则，必须先用单独的 `rg` 命令验证语法与命中结果，再放进并行工具调用。禁止未验证就直接拼进并行工具参数，避免因正则语法错误导致整批工具调用失败。

## 工程思维框架（必须遵守）

> 规则告诉你"做什么/不做什么"，思维框架告诉你"怎么想"。以下六条是所有编码任务的前置心智模型。

### 调查先行：没有调查就没有发言权

编码前必须调查现有代码库（Grep、Glob、Read），摸清家底再动手。

- 接到任何编码任务，**先搜索再动手**：用 Grep/Glob 搜索相关模块、函数、组件，确认项目中是否已有可复用的实现
- 读懂现有代码的风格、命名、分层约定，新代码必须与项目风格一致
- 禁止凭主观想象写代码，禁止照搬外部模板不看项目上下文

**惩戒**：不看现有代码就开写，等同于闭门造车——产出大概率需要返工，严禁。

### 主要矛盾优先：先抓核心，再扫外围

任何复杂任务中必有一个主要矛盾。抓住它，次要问题迎刃而解。

```
接到任务 → 识别核心问题 → 制定方案 → 集中力量解决核心问题 → 次要问题依次处理
```

- 不要眉毛胡子一把抓，一个 PR 只解决一个核心问题
- 禁止"全线出击"：同时重构三个模块、改两个接口、顺手加新功能——改了 A 破了 B，必败
...
**判断标准**：如果删掉某个抽象/某段代码，系统照常运行——那它就是过度设计的产物，删掉。一个计数器不需要工厂模式 + 观察者模式 + 依赖注入。

### 战略分解，战术严谨

**战略上藐视**：任何复杂需求都是纸老虎，拆解开来不过是一组可管理的子任务。不被庞大的需求吓倒，大系统也是一行一行写出来的。

**战术上重视**：每一行代码都认真对待。

- 改动前必须评估影响范围（牵一发而动全身）
- 关键操作必须有回滚策略
- 边界条件、异常路径、并发问题——逐一消灭，这些才是真正的敌人

### 灵活应变：小步快跑，不恋战

- 先写能跑的版本，再迭代优化；不追求一步到位的完美方案
- 接口未定义 → 先 Mock 数据开发，接口就绪后直接对接
- 发现更优方案 → 果断切换，不陷入沉没成本谬误
- 需求频繁变动 → 每次提交可用的增量，保持随时可交付

### 复用先于创造

实现新功能前，必须检索所有相关模块，最大化复用。

- 优先复用已有组件、工具函数、配置模式，不浪费已有的劳动成果
- 引入新依赖前必须确认：现有依赖是否已能满足需求
- 不拿群众一针一线：不随意引入新依赖，不修改不相关的代码，不留下无用文件

## 任务执行流程（必须遵守）

1. **定位改动边界**：确认涉及的包、平台、语言层，明确"改什么、不改什么"。
2. **建立测试基线**：改动前先执行目标包的测试/静态检查，记录已有失败数量。后续验证时只需确认"不引入新失败"，无需 stash 对照。
3. **最小实现**：优先改现有层，不新建冗余抽象。
4. **补齐验证**：至少执行目标包的测试/必要构建命令，对比基线确认无新增失败。
5. **输出变更说明**：包含改动文件、行为变化、验证结果、未覆盖风险。
...
- **禁止伪测试（pseudo-test）**：测试必须验证运行时行为，禁止编写通过 `readFileSync` 读取源文件文本再用正则 `toMatch` 断言代码结构、导入模式或 API 使用方式的测试。这类"测试"本质是静态代码风格检查，不测试任何运行时逻辑，属于 ESLint 规则或代码审查的职责，不应出现在测试套件中。
- **行数预算棘轮**：当 hook 报告文件超出行数预算时，必须通过拆分文件、提取模块等方式腾出空间，不允许通过压缩注释、合并代码行、删减空行等"挤牙膏"手段绕过预算限制。正确做法是将独立的类型定义、纯函数、常量等提取到同目录的独立文件中，主文件通过 `import` 引入。
